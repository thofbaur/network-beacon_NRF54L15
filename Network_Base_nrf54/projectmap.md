...
The code shall run on an nrf54L15 Development kit (standard overlay).
...
Upon established connection, the maximum throughput shall be negotiated. Then the text "st" shall be sent via Nordic Uart Service.
...
For the time flag, the data will be 3 bytes, uint24 in big endian format.
...
For time, then should follow "Current Timer:" and then the value of the data assuming the 3 bytes
