...
The code shall run on an nrf51422 Development kit (standard overlay) and start scanning with high frequency. 
It shall continuously scan for beacons based on nrf54l15. If a beacon with name DSA is found, it shall check the manufacturing data of the advertisment package. No company ID, pure payload.
From the 3 bytes of data the relevant id, radiostatus and network status shall be extracted according to the defined position.
With pressing button0, the device shall enter connecting mode.  If network status , cleared by the mask and rightshifted by the defined shift is larger or equal than the defined readout_level, a connection to the beacons shall be established.
...
Pressing button1 shall wait until the current connection sends finished, then disconnect not connect to any further beacons until button0 is pressed again. 

# LED
LED0 shall be active if programm is running.
LED1 shall be active if programm is in connection available mode
